...
from ...db import crud
from ...db.models import ModelFormat, ModelSource, ModelType, Quantization, VersionStatus
...
async def create_model(body: ModelCreate, session: SessionDep):
...
async def list_models(
    ...
    models, total = await crud.list_models(
        session, keyword=keyword, model_type=model_type,
...
async def import_from_hf(body: dict, session: SessionDep):
    ...
    m = await crud.create_model(
        ...
        params_size=hf_info.get("safetensors", {}).get("total", "") if isinstance(hf_info.get("safetensors"), dict) else "",
...
async def update_model(model_id: str, body: ModelUpdate, session: SessionDep):
...
async def delete_model(model_id: str, session: SessionDep, store: StoreDep):
    m = await crud.get_model(session, model_id)
    if not m:
        raise HTTPException(status_code=404, detail="Model not found")
